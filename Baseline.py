from utils.DataGenerator import Dataset, cust_collate, pad
from utils.model_helper import DEVICE, TEST_PARAM, SBERT_VERSION, MAX_SENT_LENGTH, MAX_PARA_LENGTH, BATCH_SIZE, EMB_SIZE
from utils.ModelScore import ProduceAUC
import pandas as pd
...
def generateData(batch_size):
    ...
    test_generator = torch.utils.data.DataLoader(test_set, collate_fn=cust_collate, **TEST_PARAM)

    return len(test_set), test_generator


def evaluate(encoder_model, record, generator, score_df):
...
def Baseline_eval():
    ...
    evaluate(encoder, record, data_generator, score_df)
